...
要高效使用存储驱动，需先理解 Docker 如何构建和存储镜像，以及容器如何使用这些镜像。有了这些背景，你就能更好地选择数据持久化方案，并尽量避免性能问题。

## 存储驱动与 Docker 卷

Docker 通过存储驱动保存镜像层，以及容器可写层中的数据。容器删除后，可写层不会保留，因此更适合存放运行期产生的临时数据。存储驱动在空间占用上做了优化，但（取决于具体驱动）写入性能通常低于原生文件系统，尤其是采用写时复制（copy-on-write，CoW）的驱动。写入密集型应用（如数据库）会有额外的性能开销，特别是在只读层存在大量既有数据时。

对于写入密集、需要跨容器生命周期持久化，或需在多个容器间共享的数据，请使用 Docker 卷。参见[卷](../volumes.md)一节，了解如何借助卷实现持久化并提升性能。

## 镜像与层

Docker 镜像由多层组成，每一层对应 Dockerfile 中的一条指令。除最后一层外，其余各层均为只读。示例：
...
这个 Dockerfile 共包含 4 条命令。凡是修改文件系统的命令都会创建新的一层。`FROM` 首先基于 `ubuntu:22.04` 镜像创建一层；`LABEL` 只修改镜像元数据，不产生新层；`COPY` 将当前目录的文件拷贝到镜像中；第一个 `RUN` 使用 `make` 构建应用并将结果写入新层；第二个 `RUN` 删除缓存目录，同样写入新层；最后，`CMD` 指定容器启动命令，仅修改镜像元数据，也不会产生新层。

每一层只记录相对于前一层的差异。需要注意的是，新增和删除文件都会生成新层。上例中，虽然删除了 `$HOME/.cache` 目录，但它仍保留在更早的层中，并计入镜像总体体积。参见[Dockerfile 最佳实践](/manuals/build/building/best-practices.md)与[多阶段构建](/manuals/build/building/multi-stage.md)，了解如何优化 Dockerfile 以生成更高效的镜像。

这些层自下而上叠加在一起。创建新容器时，系统会在最上方添加一个可写层（通常称为“容器层”）。运行中的容器对文件系统的所有更改（新增、修改、删除）都会写入这个很薄的可写层。下图展示了一个基于 `ubuntu:15.04` 镜像的容器层次结构。

![Layers of a container based on the Ubuntu image](images/container-layers.webp?w=450&h=300)

存储驱动负责这些层如何协同工作的具体实现。Docker 提供多种存储驱动，适用场景与优缺点各不相同。

## 容器与层

容器与镜像的关键区别在于顶部的可写层。容器中新写入或修改的数据都会落在该可写层。删除容器时，这一可写层也随之删除，底层镜像不会改变。

由于每个容器都有自己的可写层，所有更改都保存在容器层中，因此多个容器可以共享同一个底层镜像，同时各自维护独立的数据状态。下图展示了多个容器共享同一 Ubuntu 15.04 镜像的情形。

![Containers sharing the same image](images/sharing-layers.webp?w=600&h=300)

Docker 使用存储驱动管理镜像层与容器可写层的内容。各驱动实现有所差异，但都基于可堆叠的镜像层与写时复制（CoW）策略。
...
要查看正在运行容器的近似大小，可使用 `docker ps -s`。输出中有两列与大小相关：
...
所有正在运行容器在磁盘上的总体占用，由各容器的 `size` 与 `virtual size` 共同决定。如果多个容器基于同一镜像启动，总占用大致为：所有容器 `size` 之和，再加上镜像本身的大小（即任一容器的 `virtual size - size`）。

此外，容器还会通过以下方式占用磁盘空间，这些不计入上述数值：

- 由[日志驱动](/manuals/engine/logging/_index.md)保存的日志文件。若容器产生日志量很大且未配置日志轮转，这部分可能相当可观。
- 容器使用的卷与绑定挂载。
- 容器配置文件占用的磁盘空间（通常很小）。
- 写入磁盘的内存页（如果启用了交换）。
...
写时复制是一种通过共享与按需拷贝文件来提升效率的策略。若某个文件或目录已存在于镜像的较低层，其他层（包括可写层）在读取时会直接复用该文件。首次需要修改该文件时（无论在镜像构建阶段还是容器运行时），存储驱动会将其拷贝到当前层并在此基础上修改，从而最小化 I/O 并控制后续各层的体积。下面分别说明这些优势。

### 共享让镜像更小

当你使用 `docker pull` 从仓库拉取镜像，或首次基于本地尚不存在的镜像创建容器时，各层会被分别拉取并存储到 Docker 的本地存储目录（Linux 主机上通常为 `/var/lib/docker/`）。如下所示：
...
第二个镜像包含第一个镜像的所有层，并在此基础上新增了由 `COPY` 与 `RUN` 指令产生的层；运行容器时还会再叠加一个可写的容器层。由于第一个镜像的各层已在本地存在，Docker 无需再次拉取；两个镜像会共享它们的公共层。

构建完这两份 Dockerfile 后，你可以通过 `docker image ls` 与 `docker image history` 验证共享层的内容寻址（加密）ID 是否一致。
...
8. 查看各镜像的历史记录。
   ...
   BuildKit 使用自有缓存机制，无需通过中间镜像进行缓存。参见 [BuildKit](/manuals/build/buildkit/_index.md) 了解更多改进。

9. 查看各镜像包含的层

   使用 `docker image inspect` 查看各镜像中各层的内容寻址（加密）ID：
   ...
   > [!TIP]
   >
   > 可以使用 `--format` 自定义 Docker 命令输出。
   > 
   > 上述示例结合 `docker image inspect` 与 `--format` 查看各层 ID，并将其格式化为 JSON 数组。`--format` 能帮助你不借助 `awk` 或 `sed`，直接提取并格式化关键信息。更多用法参见[格式化命令与日志输出](/manuals/engine/cli/formatting.md)。示例还使用了 [`jq` 工具](https://stedolan.github.io/jq/) 对 JSON 做更友好的展示。

### 拷贝让容器更高效

启动容器时，系统会在其他层之上添加一个很薄的可写容器层。容器对文件系统的任何更改都会写入该层；未修改的文件不会被拷贝到可写层，从而让可写层尽可能小。
...
不同驱动（如 Btrfs、ZFS）在写时复制上的行为有所不同，详情见各驱动的说明章节。

写入量较大的容器会比其他容器占用更多空间，因为大多数写操作都会在可写顶层中占用新增空间。注意，即使仅修改文件元数据（如权限或所有者），也可能触发 `copy_up`，从而将该文件复制到可写层。

> [!TIP]
>
> 写入密集型应用请使用卷。
>
> 避免将写入密集型应用的数据直接存放在容器内部。尤其当只读层存在既有数据时，数据库等应用更容易出现性能问题。
> 
> 推荐使用独立于容器的 Docker 卷，它为 I/O 效率而设计；卷可在容器间共享，且不会增加容器可写层的体积。参见[使用卷](../volumes.md)了解更多。
...
2. 使用 `docker ps --size` 验证 5 个容器正在运行，并查看各自大小：
   ...
   > [!NOTE]
   >
   > 该步骤需在 Linux 机器上进行；由于需要访问 Docker 守护进程的文件存储，无法在 Docker Desktop 上完成。
...
上述示例展示了写时复制如何提升容器效率：既节省空间，又能缩短启动时间。创建容器（或从同一镜像创建多个容器）时，Docker 只需创建一个薄薄的可写容器层即可。

如果每次创建容器都要复制整套底层镜像栈，容器创建时间和磁盘占用都会显著增加。这更像虚拟机的做法——每台虚拟机都有一个或多个虚拟磁盘。[`vfs` 存储](vfs-driver.md)不提供 CoW 或其他优化，使用它时，每个容器都会得到镜像数据的完整拷贝。
